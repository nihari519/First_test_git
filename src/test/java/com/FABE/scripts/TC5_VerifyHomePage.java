...
public class TC5_VerifyHomePage extends ApplicationUtil {
	@BeforeClass
	...
	@Test
	public void testTC5_VerifyHomePage() throws Throwable {
		for (int iLoop = 2; iLoop <= inputSheet.getPhysicalNumberOfRows(); iLoop++) {
			try {
				...
				if (runStatus.trim().equalsIgnoreCase("Y") && Testcase.trim().equalsIgnoreCase("TC5_VerifyHomePage")) {
				...
				String Password = Excelobject.getCellData(SheetName, "PASSWORD", iLoop).toString();				
					...
					click(HomePageOR.PopularProduct1,"Popular Product1");
