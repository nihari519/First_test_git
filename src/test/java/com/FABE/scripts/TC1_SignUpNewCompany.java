...
public class TC1_SignUpNewCompany extends ApplicationUtil {
	...
	@Test
	public void testTC1_SignUpNewCompany() throws Throwable {
		for (int iLoop = 2; iLoop <= inputSheet.getPhysicalNumberOfRows(); iLoop++) {
			try {
				...
				if (runStatus.trim().equalsIgnoreCase("Y") && Testcase.trim().equalsIgnoreCase("TC1_SignUpNewCompany")) {
				child = extent.startTest("FABORY_SignUp", strDesc);
				...
				String Message = Excelobject.getCellData(SheetName, "MESSAGE", iLoop).toString();
					...
					navigateToWebsite();
					signUp(EmailAddress,ConfirmEmail,FirstName, LastName, CompanyName, PhoneNumber, AddressLine1, PostalCode, City, Country,Message);
					executionDelay(1000);
